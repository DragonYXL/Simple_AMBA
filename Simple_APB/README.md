...
Simple APB 是一个支持双时钟域的 APB 寄存器访问模块。当然，这是一个偏理想化的"学校"项目，核心功能就是把 SoC 片内总线发起的寄存器读写请求转发到外设端。必须说明的是：实际工程中肯定不会把RW寄存器放到内部时钟域去！！！这样非单周期的ACCESS访问会带来极大的延迟，并且APB本身讲求的就是快速寄存器访问，所以推荐做法还是shadow寄存器维护。
